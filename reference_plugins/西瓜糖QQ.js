const CONFIG = {
  apiKey: "78ba562b6b7de94edb2f4465f1a2a1feaffe9878e3b98385c4d7e7b4cf98a524",
  enableCustomAPI: true,
  debugMode: false,
  requestTimeout: 10000,
  maxRetryCount: 3,
  qualityPriority: ["super", "high", "standard", "low"]
};
...
function $d25ff3d008f9e7e0$var$getApiKey() {
  ...
  if (typeof env !== 'undefined' && env.getUserVariables) {
    ...
    return (vars && vars.apiKey) || "";
  }
  
  if (typeof window !== 'undefined' && window.pluginConfigs && window.pluginConfigs.qqmusic && window.pluginConfigs.qqmusic.apiKey) {
    return window.pluginConfigs.qqmusic.apiKey;
  }
...
async function $d25ff3d008f9e7e0$var$getMusicInfoFromCustomAPI(songmid) {
  const apiKey = $d25ff3d008f9e7e0$var$getApiKey();
  
  if (!apiKey || apiKey.trim() === "") {
    throw new Error("请先在用户变量中配置API密钥");
  }
  
  try {
    const url = `https://api.nki.pw/API/music_open_api.php?apikey=${encodeURIComponent(apiKey)}&mid=${songmid}`;
    ...
    if (response.data && response.data.song_mid) {
...
async function $d25ff3d008f9e7e0$var$getLyric(musicItem) {
    try {
        const apiData = await $d25ff3d008f9e7e0$var$getMusicInfoFromCustomAPI(musicItem.songmid);
        
        if (apiData.song_lyric) {
            return {
                rawLrc: apiData.song_lyric,
                translation: apiData.lyric || ""
            };
        }
        
        throw new Error("API未返回歌词");
    } catch (error) {
...
const $d25ff3d008f9e7e0$var$qualityMap = {
    low: "standard",
    standard: "standard",
    high: "sq",
    super: "sq"
...
async function $d25ff3d008f9e7e0$var$getMediaSource(musicItem, quality) {
  ...
  try {
    ...
    let playUrl;
    const qualityLevel = $d25ff3d008f9e7e0$var$qualityMap[quality] || "standard";
    
    if (qualityLevel === "sq" && apiData.song_play_url_sq) {
      playUrl = apiData.song_play_url_sq;
    } else if (apiData.song_play_url) {
      playUrl = apiData.song_play_url;
    } else {
      throw new Error("未找到对应音质的播放链接");
    }
...
const $d25ff3d008f9e7e0$var$pluginInstance = {
    ...
    author: "玥然OvO",
    version: "1",
    srcUrl: "https://raw.gitcode.com/Crystim/mfp/raw/main/%E8%A5%BF%E7%93%9C%E7%B3%96QQ.js",
    ...
    userVariables: [
        {
            key: "apiKey",
            name: "API密钥",
            hint: "请输入自定义API的密钥，获取地址：https://api.nki.pw"
        }
    ],
